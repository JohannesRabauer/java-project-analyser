...
@ApplicationScoped
public class GetProjectSummaryHandler implements McpToolHandler {

    private final AnalysisPipelineService pipelineService;

    public GetProjectSummaryHandler(AnalysisPipelineService pipelineService) {
        this.pipelineService = pipelineService;
    }

    @Override
    public String toolName() { return "get_project_summary"; }

    @Override
    public String description() {
        return "Pre-computed executive summary, business purpose, and system classification. Instant response.";
    }
    ...
    public ToolCallResult handle(JsonNode args) {
        ...
        var result = pipelineService.getPhaseResult(jobId, 4);
        if (result.isEmpty()) {
